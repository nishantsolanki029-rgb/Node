...
module.exports.updateProduct = async (req, res) => {
    try {
        ...
        const updatedProduct = await productService.updateProduct({ productId, name, description, stock, price, discount, isNewProduct, sku, images, brand, category })

        return res.status(200).json({ messge: "Product Updated Successfully", updatedProduct })
    } catch (error) {
        return res.status(400).json({ message: error.message })
    }
}
